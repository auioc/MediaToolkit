...
function ProcessSingle([System.IO.FileInfo]$inputFile, [string]$outputFile, [bool]$NoOutput, [bool]$NoBanner) {
...
function Main {
    ...
    $files | ForEach-Object {
        ...
        if (-not $DryRun) {
            ProcessSingle $_ $outputFile $NoOutput $NoBanner
        }
...
Main $InputPath $OutputPath $IncludeExtensions $Recurse $Depth $DryRun $NoOutput $NoOverwrite $NoBanner
